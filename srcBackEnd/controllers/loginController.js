...
exports.login = async (req, res) => {
  ...
  if (response.length !== 0) {
    ...
    const token = jwt.sign(payload, response[0].secret, options);
    res.send({
      OK: 1,
...
exports.logout = async (req, res) => {
  ...
  try {
    ...
    const sql = `UPDATE Users SET secret = ? WHERE id = ${idUser}`;
    const values = newSecret;
    const response = await doQuery(sql, values);
    res.send({
      OK: 1,
    ...
    try {
      ...
      if (response.length !== 0) {
        ...
        try {
          ...
          try {
            const newSecret = nanoid(10);
            sql = `UPDATE Users SET secret = "${newSecret}" , pass= ? WHERE email = "${email}"`;
            values = pass;
